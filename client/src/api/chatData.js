...
export const useCreateChatMutation = () => {
  ...
  const { isLoading, isSuccess, error, mutate } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['groupedChatData'] });
    },
  });
...
export const useGroupedChatData = () => {
  const { isPending, error, data } = useQuery({
    queryKey: ['groupedChatData'],
    queryFn: () => apiService.get('/chats/grouped').then((res) => res.data),
  });
  return { isPending, error, data };
};
